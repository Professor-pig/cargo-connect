...
def wait_for_button_clicked():
    ev3.light.on(parameters.Color.RED)
    while True:
        button = ev3.buttons.pressed()
        if button:
            ev3.light.on(parameters.Color.GREEN)
            break
...
def route_3():
    ...
    if route_3_stage_1:
        ...
        robot.advance_without_acceleration(62.5, 600, 0.79)
        time.sleep(0.5)
        wait_for_button_clicked()
        robot.left_motor.spin_for_deg(2200)
        time.sleep(0.5)
        robot.advance_without_acceleration(3, 200, 0.9) 
...
def route_1():
    ...
    time.sleep(0.1)
    robot.turn(32)
    robot.advance(84, 400, 0.98)
    time.sleep(0.1)
    robot.turn(88)
    put_down_fork()
    robot.advance(23, 400, 0.979)
    lower_left_motor()
    
    threading.Thread(target=lift_fork).start()
    threading.Thread(target=raise_left_motor).start()
    robot.retreat(42, 300)

    # push train
    lower_left_motor()
    robot.advance(41, 1000, 0.94)    
    time.sleep(0.1)
    robot.advance(1, 100, 0.94)    
    time.sleep(0.5)
...
def route_4():
    ...
    time.sleep(0.1)
    crane_pusher_go_out()
    threading.Thread(target=lambda: robot.right_motor.spin_for_deg(-4200)).start()
...
if debug:
    robot.left_motor.spin_for_deg(-1650)
else:
